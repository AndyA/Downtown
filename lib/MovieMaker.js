...
module.exports = (function() {
  ...
  var dynamicPropertyMixin = (function() {
    ...
    return {
      ...
      evaluateInContext: function(name, prop) {
        ...
        pctx.eval = true;
        pctx.cache = prop instanceof exports.DynamicPropertyBase ? prop.evaluate(ctx) : prop.apply(this, [ctx]);
        delete pctx.eval;
        return pctx.cache;
      },
      ...
      bindProperty: function(name, prop) {
        if (prop instanceof exports.DynamicPropertyBase) {
          this.bindProperty(name, function(ctx) {
            return prop.evaluate(ctx);
          });
          return;
        }
        ...
        if (prop instanceof Function) {
          Object.defineProperty(this, name, {
            get: function() {
              var ctx = this.getContext();
              if (!ctx._prop[name]) ctx._prop[name] = {};
              var pctx = ctx._prop[name];
              var cache = pctx.cache;
              ...
              pctx.eval = true;
              pctx.cache = prop.apply(this, [ctx]);
              delete pctx.eval;
              return pctx.cache;
            }
          })
  ...
  exports.CircleProperty = defineClass(function(params) {
    ...
    this.bindValue('x', function(ctx) {
      var angle = (ctx.portion * this.frequency + this.phase) * Math.PI * 2;
      return (Math.sin(angle) + this.offset_x) * this.amplitute * this.scale_y;
    });

    this.bindValue('y', function(ctx) {
      var angle = (ctx.portion * this.frequency + this.phase) * Math.PI * 2;
      return (Math.cos(angle) + this.offset_y) * this.amplitute * this.scale_y;
    });
  },
  new exports.DynamicPropertyBase(), {
    evaluate: function(ctx) {
