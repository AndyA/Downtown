...
module.exports = (function() {
  ...
  var dynamicPropertyMixin = (function() {
    ...
    return {
      ...
      evaluateInContext: function(name, prop) {
        ...
        if (isMagic(val)) return this.evaluateInContext(name, val);

        return pctx.cache = val;
      },

      bindProperty: function(name, prop) {
        var getter = isMagic(prop) ?
        function() {
          return this.evaluateInContext(name, prop)
        } : function() {
          return prop
        }
  ...
  exports.CircleProperty = defineClass(function(params) {
    ...
    var self = this; // bindValue needs this
    self.bindValue('x', function(ctx) {
      var angle = (ctx.portion * self.frequency + self.phase) * Math.PI * 2;
      //      console.log("x: portion=" + ctx.portion + ", angle=" + angle + " (" + (angle * 180 / Math.PI) + " degrees)");
      return (Math.sin(angle) + self.offset_x) * self.amplitute * self.scale_x;
    });

    self.bindValue('y', function(ctx) {
      var angle = (ctx.portion * self.frequency + self.phase) * Math.PI * 2;
      //      console.log("y: portion=" + ctx.portion + ", angle=" + angle + " (" + (angle * 180 / Math.PI) + " degrees)");
